...
class plgSystemCorreios extends JPlugin {
	...
	private function updateWeightCosts() {
		...
		if( preg_match( '|<table class="conteudo-tabela">.+?<td>REGISTRO NACIONAL.+?([1-9][0-9.,]+).+?<td>REGISTRO MÓDICO.+?([1-9][0-9.,]+)|s', $tracking, $m ) ) {
			...
			if( preg_match( '|Mais *de *20 *até *50</td>\s*(.+?)<tr class="rodape-tabela">|si', $weights, $m ) ) {
