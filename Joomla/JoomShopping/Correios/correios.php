...
class plgSystemCorreios extends JPlugin {
	...
	public static function makeCartaPackages( $items ) {
		...
		foreach( $items as $i => $item ) {
			...
			if( $items[$i]->weight > 0 ) $tmp[] = $items[$i];
		}
		$items = $tmp;
	...
	private function updateWeightCosts() {
		...
		if( preg_match( '|<table class="conteudo-tabela">.+?<td><strong>REGISTRO NACIONAL.+?([1-9][0-9.,]+).+?<td><strong>REGISTRO MÓDICO.+?([1-9][0-9.,]+)|is', $tracking, $m ) ) {
			...
			if( preg_match( '|<td>\s*Mais *de *20 *.+?(<td>.+?)<tr class="rodape-tabela">|is', $weights, $m ) ) {
