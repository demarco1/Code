<?php
/**
* @version      1.0.0 22.08.2015
* @author       Aran Dunkley & Elisabeth Medeiros de Oliveira
* @package      Jshopping
* @copyright    Copyright (C) 2015 Ligmincha Brasil
* @license      GNU/GPL
*/

define('_JSHOP_DETAIL', 'Detalhes');
define('_JSHOP_QUANTITY', 'Quantidade');
define('_JSHOP_ADD_TO_CART', 'Adicionar ao carrinho');
define('_JSHOP_ADD_TO_WISHLIST', 'Adicionar à lista de desejos');
define('_JSHOP_WISHLIST_IS_EMPTY', 'Sua lista de desejos está vazia');
define('_JSHOP_ORDER_BY', 'Ordenar por');
define('_JSHOP_DISPLAY_NUMBER', 'Mostra');
define('_JSHOP_TABS_MAIN', 'Primeiro');
define('_JSHOP_TABS_VIDEO', 'Vídeo');
define('_JSHOP_PRODUCT', 'Produto');
define('_JSHOP_PRICE', 'Preço');
define('_JSHOP_ALL', 'Todos');
define('_JSHOP_ADDED_TO_CART', 'Produto foi adicionado ao carrinho');
define('_JSHOP_ADDED_TO_WISHLIST', 'produto foi adicionado ao carrinho à lista de desejos');
define('_JSHOP_ERROR_ADDED_TO_CART', 'Acrescentando artigo ao carrinho de erro');
define('_JSHOP_PRODUCT_COUNT', 'Produto(s)');
define('_JSHOP_PRODUCTS', 'Produtos');
define('_JSHOP_CART', 'Carrinho');
define('_JSHOP_NO_PRODUCTS_CART', 'Seu carrinho de compras está vazio');
define('_JSHOP_WISHLIST', 'Lista de desejos');
define('_JSHOP_REMOVE', 'Excluir');
define('_JSHOP_ITEM', 'Item');
define('_JSHOP_NUMBER', 'Quantidade');
define('_JSHOP_PROD_NUMBER', 'Número de item');  
define('_JSHOP_SINGLEPRICE', 'Preço único');
define('_JSHOP_SINGLEPRICE_NETTO', 'Net');
define('_JSHOP_IMAGE', 'Imagem');
define('_JSHOP_TAX', 'Incluindo impostos');
define('_JSHOP_PRICE_TOTAL', 'Total');
define('_JSHOP_PRICE_SUBTOTAL', 'Subtotal');
define('_JSHOP_UPDATE_CART', 'Atualizar carrinho');
define('_JSHOP_CHECKOUT', 'Finalizar compra');
define('_JSHOP_RABATT', 'Código de desconto');
define('_JSHOP_RABATT_ACTIVE', 'Ativar');
define('_JSHOP_RABATT_NON_SUPPORT', 'Código de desconto não é suportado');
define('_JSHOP_RABATT_NON_CORRECT', 'Você digitou um código errado');
define('_JSHOP_RABATT_USED', 'Você digitou um código usado');
define('_JSHOP_BACK_TO_SHOP', 'Voltar para a loja');
define('_JSHOP_HAVE_NOT_ACCOUNT', 'Cliente novo');
define('_JSHOP_HAVE_ACCOUNT', 'login de cliente');
define('_JSHOP_REGISTRATION', 'Cadastre-se no site');
define('_JSHOP_REGISTER', 'Por favor cadastre-se');
define('_JSHOP_PL_LOGIN', 'Por favor entre');
define('_JSHOP_LOGIN', 'Login');
define('_JSHOP_REMEMBER_ME', 'Lembre de mim');
define('_JSHOP_EMAIL', 'E-mail');
define('_JSHOP_PASSWORT', 'Senha');
define('_JSHOP_LOST_PASSWORD', 'Senha perdida');
define('_JSHOP_LOST_PASSWORD_DESC', 'Digite seu usuário e e-mail e clique no botão Enviar senha.<br />Você receberá uma nova senha brevemente.');
define('_JSHOP_SEND_PASSWORD', 'Enviar senha');
define('_JSHOP_PASS_ERROR', 'Nenhum usuário correspondente foi encontrado');
define('_JSHOP_NEWPASS_SENT', 'Nova senha criada e enviada!');
define('_JSHOP_LOGOUT', 'Sair');
define('_JSHOP_MANUFACTURER', 'Fabricante');
define('_JSHOP_CATEGORY', 'Categoria');
define('_JSHOP_ACCESS_DENIED', 'Acesso negado');
define('_JSHOP_REQUIRED', 'Requerido');
define('_JSHOP_BACK', 'Anterior');
define('_JSHOP_DELETE', 'Excluir');
define('_JSHOP_CONFIRM_REMOVE', 'Realmente excluir?');

// Messages
define('_JSHOP_REGWARN_INUSE', 'This username/password already in use. Please try another.');
define('_JSHOP_REGWARN_NAME','Please enter your first name.');
define('_JSHOP_REGWARN_LNAME','Please enter your last name.');
define('_JSHOP_REGWARN_UNAME','Please enter username.');
define('_JSHOP_REGWARN_MAIL','Please enter a valid e-mail address.');
define('_JSHOP_REGWARN_EMAIL_INUSE','This e-mail is already registered. If you forgot the password, click on the Lost your Password link and a new password will be sent to you.');
define('_JSHOP_VALID_AZ09',"Please enter a valid %s. No spaces, no less than %d characters and should contain only 0-9,a-z,A-Z");
define('_JSHOP_REGWARN_STREET', 'Please enter a street.');
define('_JSHOP_REGWARN_HOME', 'Please enter a home.');
define('_JSHOP_REGWARN_APARTMENT', 'Please enter a apartment.');
define('_JSHOP_REGWARN_ZIP', 'Please enter a valid postal code');
define('_JSHOP_REGWARN_CITY', 'Please enter city');
define('_JSHOP_REGWARN_STATE', 'Please enter a state');
define('_JSHOP_REGWARN_COUNTRY', 'Please select a country');
define('_JSHOP_REGWARN_TITLE', 'Please select a title');
define('_JSHOP_REGWARN_PHONE', 'Please enter a phone');
define('_JSHOP_REGWARN_NAME_DELIVERY','Please enter delivery first name');
define('_JSHOP_REGWARN_LNAME_DELIVERY','Please enter delivery last name');
define('_JSHOP_REGWARN_MAIL_DELIVERY','Please enter a valid delivery e-mail address');
define('_JSHOP_REGWARN_STREET_DELIVERY', 'Please enter a delivery street');
define('_JSHOP_REGWARN_ZIP_DELIVERY', 'Please enter a valid delivery postal code');
define('_JSHOP_REGWARN_STATE_DELIVERY', 'Please enter a delivery state');
define('_JSHOP_REGWARN_HOME_DELIVERY', 'Please enter a delivery home');
define('_JSHOP_REGWARN_APARTMENT_DELIVERY', 'Please enter a delivery apartment');
define('_JSHOP_REGWARN_COUNTRY_DELIVERY', 'Please select a delivery country');
define('_JSHOP_REGWARN_TITLE_DELIVERY', 'Please select a delivery title');
define('_JSHOP_REGWARN_PHONE_DELIVERY', 'Please enter a delivery phone');
define('_JSHOP_REGWARN_FIRMA_NAME', 'Please enter company');
define('_JSHOP_REGWARN_FIRMA_NAME_DELIVERY', 'Please enter delivery company');
define('_JSHOP_REGWARN_CLIENT_TYPE', 'Please enter Customer type');
define('_JSHOP_REGWARN_FAX', 'Please enter fax');
define('_JSHOP_REGWARN_FAX_DELIVERY', 'Please enter delivery fax');
define('_JSHOP_REGWARN_CITY_DELIVERY', 'Please enter delivery city');
define('_JSHOP_REGWARN_FIRMA_CODE', 'Please enter company code');
define('_JSHOP_REGWARN_TAX_NUMBER', 'Please enter tax number');
define('_JSHOP_REGWARN_MOBIL_PHONE', 'Please enter mobile phone');
define('_JSHOP_REGWARN_MOBIL_PHONE_DELIVERY', 'Please enter delivery mobile phone');
define('_JSHOP_REGWARN_EXT_FIELD_1', 'Please enter ext. field 1');
define('_JSHOP_REGWARN_EXT_FIELD_2', 'Please enter ext. field 2');
define('_JSHOP_REGWARN_EXT_FIELD_3', 'Please enter ext. field 1');
define('_JSHOP_REGWARN_EXT_FIELD_1_DELIVERY', 'Please enter delivery  ext. field 1');
define('_JSHOP_REGWARN_EXT_FIELD_2_DELIVERY', 'Please enter delivery  ext. field 2');
define('_JSHOP_REGWARN_EXT_FIELD_3_DELIVERY', 'Please enter delivery  ext. field 3');
define('_JSHOP_REGWARN_PASSWORD_NOT_MATCH', 'Passwords not match');
define('_JSHOP_REGWARN_PASSWORD', 'Please enter password');
define('_JSHOP_PLEASE_ENTER_X', 'Please enter %s');

define('_JSHOP_REGWARN_ERROR_DATABASE', 'Error by saving in database');

define('_JSHOP_NEXT', 'Next');
define('_JSHOP_GO', 'Go');
define('_JSHOP_USEND_MSG_ACTIVATE', 'Hello %s.
Thank you for registering at %s. Your account is created and must be activated before you can use it.
To activate the account click on the following link or copy-paste it in your browser:
%s

After activation you may login to %s using the following username and password:

Username - %s
Password - %s');
define('_JSHOP_USEND_MSG', "Hello %s.

Thank you for registering at %s.

You may now login to %s using the username and password you registered with.");

// Registration
define('_JSHOP_REG_TITLE', 'Título');
define('_JSHOP_USERNAME', 'Usuário');
define('_JSHOP_F_NAME', 'Primero nome');
define('_JSHOP_L_NAME', 'Sobrenome');
define('_JSHOP_FULL_NAME', 'Nome completo');
define('_JSHOP_FIRMA_NAME', 'Empresa');
define('_JSHOP_HOME', 'Home');
define('_JSHOP_APARTMENT', 'Apartamento');
define('_JSHOP_STREET_NR', 'Rua / Número / Complemento');
define('_JSHOP_ZIP', 'CEP');
define('_JSHOP_STATE', 'Estado');
define('_JSHOP_CITY', 'Cidade');
define('_JSHOP_COUNTRY', 'País');
define('_JSHOP_TELEFON', 'Telefone');
define('_JSHOP_MOBIL_PHONE', 'Celular');
define('_JSHOP_FAX', 'Fax');
define('_JSHOP_PASSWORD', 'Senha');
define('_JSHOP_PASSWORD_2', 'Confirme a senha');
define('_JSHOP_EMAIL2', 'Confirme o e-mail');
define('_JSHOP_SEND_REGISTRATION', 'Cadastre-se');
define('_JSHOP_USER_EXIST', "User with the name %s already exists\nPlease enter another username");
define('_JSHOP_USER_EXIST_EMAIL', "User with the E-mail %s already exists");
define('_JSHOP_REG_SELECT', 'Select');
define('_JSHOP_FIRMA_CODE', 'Сompany code');
define('_JSHOP_VAT_NUMBER', 'TAX ID');
define('_JSHOP_EXT_FIELD_1', 'Campo adicional 1');
define('_JSHOP_EXT_FIELD_2', 'Campo adicional 2');
define('_JSHOP_EXT_FIELD_3', 'Campo adicional 3');
define('_JSHOP_CLIENT_TYPE', 'Tipo de Cliente');
define('_JSHOP_PRIVAT_CLIENT', 'Pessoa');
define('_JSHOP_FIRMA_CLIENT', 'Empresa');

define('_JSHOP_SORT_ALPH', 'Nome');
define('_JSHOP_SORT_PRICE', 'Preço');
define('_JSHOP_SORT_DATE', 'Data');
define('_JSHOP_SORT_MANUAL', 'Predefined');
define('_JSHOP_SORT_RATING', 'Rating');
define('_JSHOP_SORT_POPULAR', 'Popular');

define('_JSHOP_RABATT_VALUE', 'Discount');
define('_JSHOP_RELATED_PRODUCTS', 'Related products');
define('_JSHOP_NO_SELECT_PRODUCT', 'You did not select product');
define('_JSHOP_YES', 'Sim');
define('_JSHOP_NO', 'Não');
define('_JSHOP_NOT', 'Not');
define('_JSHOP_DELIVERY_ADRESS', 'Outro endereço de envio');
define('_JSHOP_AGB', 'Terms of Service');
define('_JSHOP_RETURN_POLICY', 'Return policy');
define('_JSHOP_SELECT_VALUE', 'Select value');

// Payment
define('_JSHOP_TRANSACTION_END','Status do pedido para transações completas');
define('_JSHOP_TRANSACTION_PENDING','Status do pedido para pagamentos pendentes');
define('_JSHOP_TRANSACTION_FAILED','Status do pedido para transações com erros');
define('_JSHOP_CHECK_DATA_RETURN', 'Verifique os dados após voltar do sistema de pagamento');
define('_JSHOP_ACCOUNT_HOLDER', 'Nome do usuário da conta');
define('_JSHOP_ACCOUNT_NUMBER', 'Agência');
define('_JSHOP_BIC', 'Código identificador do banco');
define('_JSHOP_BANK', 'Banco');
define('_JSHOP_BIC_BIC', 'CNPJ');
define('_JSHOP_IBAN', 'Favorecido/Razão Social Pessoa Jurídica');
define('_JSHOP_ERROR_PAYMENT', 'Ero no método pagamento');
define('_JSHOP_ORDER_FINISH', 'Confirmar compra');
define('_JSHOP_ERROR_PAYMENT_DATA', 'Por favor corrija as informações de pagamento');
define("_JSHOP_HI","Olá");
define('_JSHOP_ORDER_STATUS_CHANGE_SUBJECT','Status do pedido número %s mudou');
define('_JSHOP_ORDER_STATUS_CHANGE_TITLE', 'Mudança de status');
define('_JSHOP_YOUR_ORDER_STATUS_CHANGE', 'O status do seu pedido número %s foi modificado');
define('_JSHOP_NEW_STATUS_IS', 'O novo status é');
define('_JSHOP_ORDER_CANCELED', 'Ordem foi cancelada');
define('_JSHOP_COMMENT_YOUR_ORDER', 'Comentários sobre seu pedido');

// Step order
define('_JSHOP_STEP_ORDER_2', 'Endereço');
define('_JSHOP_STEP_ORDER_3', 'Método de pagamento');
define('_JSHOP_STEP_ORDER_4', 'Método de frete');
define('_JSHOP_STEP_ORDER_5', 'Confirmar compra');

// End order
define('_JSHOP_ADD_INFO', 'As anotações abaixo vão ser adicionadas na sua compra:');
define('_JSHOP_FINISH_DELIVERY_ADRESS', 'Endereço de envio');
define('_JSHOP_FINISH_SHIPPING_METHOD', 'Método de frete');
define('_JSHOP_FINISH_PAYMENT_METHOD', 'Método de pagamento');
define('_JSHOP_THANK_YOU_ORDER', '<h1>Obrigado pela sua compra!</h1>
<h2>Um recibo foi enviado para seu e-mail.</h2><br />
<h3>Nossos dados bancários</h3>
<b>Nome do banco: </b>Banco Bradesco S.A.<br />
<b>Código identificador do banco: </b>237<br />
<b>Agência: </b>6613-3<br />
<b>Conta corrente:</b> 8484-0<br />
<b>Favorecido: </b>Christiane Meng de Azevedo 26255587843<br />
<b>CNPJ: </b>21.105.170/0001-00<br /><br />
<b>Pedimos a gentileza de enviar o comprovante de pagamento por e-mail.</b>');
define('_JSHOP_ORDER_PLACED', 'Your order has been successfully placed!'); // not used

// E-mail
define('_JSHOP_EMAIL_THANK_YOU', 'Thank you for shopping with us. Your order information follows.'); // not used
define('_JSHOP_EMAIL_PURCHASE_ORDER', 'Pedido de compra');
define('_JSHOP_DESCRIPTION', 'Descrição');
define('_JSHOP_ORDER_NUMBER', 'Número de compra');
define('_JSHOP_ORDER_DATE', 'Data do pedido');
define('_JSHOP_ORDER_FROM', 'Pedido efetuada em');
define('_JSHOP_ORDER_SHORT_NR', 'Não');
define('_JSHOP_ORDER_STATUS', 'Status do pedido');
define('_JSHOP_CUSTOMER_INFORMATION', 'Informação do cliente');
define('_JSHOP_ORDER_HISTORY', 'Historial do pedido');
define('_JSHOP_ORDER_COMMENT', 'Comentário');
define('_JSHOP_NEW_ORDER', 'Novo pedido # %s (%s)');
define('_JSHOP_NEW_ORDER_V', 'Novo pedido # %s ');
define('_JSHOP_EMAIL_BILL_TO', 'Cobrança para');
define('_JSHOP_EMAIL_BILL', 'Bill');
define('_JSHOP_EMAIL_SHIP_TO', 'Enviar para');
define('_JSHOP_ORDER_ITEMS', 'Itens do pedido');
define('_JSHOP_NAME_PRODUCT', 'Nome do prodto');
define('_JSHOP_EAN_PRODUCT', 'Código do produto');
define('_JSHOP_TOTAL', 'Total');
define('_JSHOP_SUBTOTAL', 'Subtotal');
define('_JSHOP_SUBTOTAL_NETTO', 'Subtotal Net');
define('_JSHOP_VAT', 'TAX');
define('_JSHOP_ENDTOTAL', 'Total');
define('_JSHOP_ENDTOTAL_INKL_TAX', 'Total incl. TAX');
define('_JSHOP_BENEF_BANK', 'Beneficiary Bank info');
define('_JSHOP_BENEF_BIC', 'Código identificador do banco');
define('_JSHOP_BENEF_CONTO', 'Agência');
define('_JSHOP_BENEF_IBAN', 'Favorecido');
define('_JSHOP_BENEF_SWIFT', 'Código SWIFT');
define('_JSHOP_INTERM_BANK', 'Intermediary Bank');
define('_JSHOP_INTERM_NAME', 'Nome');
define('_JSHOP_BENEF_BANK_NAME','Nome do banco');
define('_JSHOP_BENEF_PAYEE','Conta corrente'); 
define('_JSHOP_INTERM_SWIFT', 'Código SWIFT');
define('_JSHOP_SHIPPING_PRICE', 'Preço de envio');
define('_JSHOP_WEIGHT_UNIT', 'Kg');
define('_JSHOP_WEIGHT', 'Weight');
define('_JSHOP_WEIGHT_PRODUCTS', 'Weight products');
define('_JSHOP_COUPON_DISCOUNT', 'Coupon discount');
define('_JSHOP_CUSTOMER_NOTE', 'Customer note');
define('_JSHOP_PAYMENT_INFORMATION', 'Informação do pagamento');
define('_JSHOP_SHIPPING_INFORMATION', 'Informação de envio');
define('_JSHOP_CONTACT_PHONE','Phone');
define('_JSHOP_CONTACT_FAX','Fax');

// Account
define('_JSHOP_SAVE', 'Save');
define('_JSHOP_MY_ACCOUNT', 'Minha conta');
define('_JSHOP_ACCOUNT_UPDATE', 'Account was updated');
define('_JSHOP_NEW_PASSWORD', 'Nova senha');
define('_JSHOP_NEW_PASSWORD_2', 'Confirmar nova senha');
define('_JSHOP_ADVANCED_SEARCH', 'Advanced search');
define('_JSHOP_NO_SEARCH_RESULTS', 'No search results found');
define('_JSHOP_EDIT_DATA', 'Editar dado');
define('_JSHOP_SHOW_ORDERS', 'Mostrar pedidos');
define('_JSHOP_NO_ORDERS', 'Não efectuou nenhum pedido');
define('_JSHOP_CANCEL_ORDER', 'Cancelar pedido');

// Search
define('_JSHOP_SEARCH', 'Buscar');
define('_JSHOP_SEARCH_CATEGORIES', 'Categorias');
define('_JSHOP_SEARCH_ALL_CATEGORIES', 'Todas categorias');
define('_JSHOP_SEARCH_INCLUDE_SUBCAT', 'Include subcategories');
define('_JSHOP_SEARCH_MANUFACTURERS', 'Fabricantes');
define('_JSHOP_SEARCH_ALL_MANUFACTURERS', 'Todos os fabricantes');
define('_JSHOP_SEARCH_PRICE_FROM', 'Price from');
define('_JSHOP_SEARCH_PRICE_TO', 'Price to');
define('_JSHOP_SEARCH_DATE_FROM', 'Date from');
define('_JSHOP_SEARCH_DATE_TO', 'Date to');

// Reviews
define('_JSHOP_COMMENT', 'Comentário');
define('_JSHOP_NEW_COMMENT', 'Novo comentário');
define('_JSHOP_REVIEWS', 'Comentários');
define('_JSHOP_REVIEW_NOT_LOGGED', 'Please log in to write comments.');
define('_JSHOP_ADD_REVIEW_PRODUCT', 'Write a comment for this product!');
define('_JSHOP_REVIEW_USER_NAME', 'Nome');
define('_JSHOP_REVIEW_USER_EMAIL', 'E-mail');
define('_JSHOP_REVIEW_REVIEW', 'Please write comment');
define('_JSHOP_REVIEW_MARK_PRODUCT', 'Rating for product');
define('_JSHOP_REVIEW_SUBMIT', 'Submit comment');
define('_JSHOP_REVIEW_MARK', 'Rating');

//Downloads
define('_JSHOP_DOWNLOAD_DEMO', 'Download demo');
define('_JSHOP_DOWNLOAD', 'Download');
define('_JSHOP_FOR_DOWNLOAD_ORDER_MUST_BE_PAID', 'Order must be paid');
define('_JSHOP_NUMBER_DOWNLOADS_FILE_RESTRICTED', 'Number downloads of file is restricted');
define('_JSHOP_TIME_DOWNLOADS_FILE_RESTRICTED', 'Time downloads of file is restricted');

// No price
define('_JSHOP_NO_PRICE', 'No price');  
define('_JSHOP_NO_PRICE_ADMIN_MESSAGE', 'Please send to admin message');
define('_JSHOP_NO_PRICE_MESSAGE', 'Message');
define('_JSHOP_NO_PRICE_SEND_TO_ADMIN', 'Send');  
define('_JSHOP_PRICE_REQUEST', 'Price request');
define('_JSHOP_SEND_MAIL_GOOD', 'Mail send successfull');  
define('_JSHOP_ERROR_SENDING_MAIL', 'Error sending mail');

// new version vars
define('_JSHOP_PAYMENT_PRODUCT_IN_SITE', 'Payment for goods in the shop %s');
define('_JSHOP_REDIRECT_TO_PAYMENT_PAGE', 'Redirecting to the Payment page, please wait...');
define('_JHOP_ERROR_STEP','Error step');
define('_JSHOP_ERROR_DATA',"Error data");
define('_JSHOP_ERROR_ORDER_HASH',"Error hash");
define("_JSHOP_CHECKOUT_ADDRESS","Address");
define("_JSHOP_CHECKOUT_PAYMENT","Payment");
define("_JSHOP_CHECKOUT_SHIPPING","Shipping");
define("_JSHOP_CHECKOUT_PREVIEW","Confirmar pedido");
define("_JSHOP_CHECKOUT_FINISH","Order completed");
define('_JSHOP_INC_TAX', 'incl. TAX');   
define('_JSHOP_PLUS_TAX', 'plus TAX');   
define('_JSHOP_ERROR_ORDERID', 'Error Order ID');
define('_JSHOP_PAYMENT_CANCEL', 'Cancel payment');
define('_JSHOP_PAYMENT_CANCELED', 'Payment canceled');
define('_JSHOP_PAYMENT_CANCEL_ORDER_NUM', 'Payment cancel. Order number %s');
define('_JSHOP_SESSION_FINISH', 'Error session time finish');
define('_JSHOP_SEARCH_RESULT', 'Search');
define('_JSHOP_SEARCH_TEXT', 'Text');
define('_JSHOP_REMOVE_TO_CART', 'Add to basket');
define('_JSHOP_MR', 'Mr.');
define('_JSHOP_MS', 'Ms.');
define('_JSHOP_FIRMA', 'Company');
define('_JSHOP_ENTER_CORRECT_INFO_REVIEW','Enter correct data comment');
define('_JSHOP_YOUR_REVIEW_SAVE','Your comment saved and will be shown after the confirmation Administrator');
define('_JSHOP_YOUR_REVIEW_SAVE_DISPLAY','Your comment saved');
define('_JSHOP_ERROR_PAYMENT_FILE_NOT_EXIST','Payment method "%s" error, file "%s" not exist');
define('_JSHOP_GO_TO_CART','Go to cart');
define('_JSHOP_GO_TO_WISHLIST','Go to wishlist');
define('_JSHOP_PRODUCTS_X','Products');
define('_JSHOP_INSTALLED_NEW_LANGUAGES','Installed new languages');
define('_JSHOP_OLD_PRICE','Old price');
define('_JSHOP_DETAILS','Details');
define('_JSHOP_RATING','Rating');
define('_JSHOP_FOR_USE_COUPON_PLEASE_LOGIN','Please login to use coupons');
define('_JSHOP_EAN', 'Código');
define('_JSHOP_PAYMENT_NUMBER', 'Order number %s');
define('_JSHOP_ORDER_WITHOUT_REGISTER_CLICK', 'To <b>Order without registration</b>, please click');
define('_JSHOP_HERE', 'aqui');
define('_JSHOP_PRICE_FOR_QTY', 'Preços');
define('_JSHOP_PER_PIECE', 'per piece');
define('_JSHOP_SELECT_PRODUCT_OPTIONS', 'Please select the product options');
define('_JSHOP_ERROR_EXIST_QTY_PRODUCT', 'Some products can not be delivered in the quantity ordered<br/>The maximum order quantity is %s.');
define('_JSHOP_ERROR_EXIST_QTY_PRODUCT_IN_CART', 'Some products can not be delivered in the quantity ordered<br/>Your cart already has %s products.<br/>The maximum order quantity is %s.');
define('_JSHOP_ERROR_EXIST_QTY_PRODUCT_BASKET', 'The product "%s" maximum order quantity is %s.');
define('_JSHOP_BUY', 'Comprar');
define('_JSHOP_PRODUCT_NOT_AVAILABLE', 'Not available');
define('_JSHOP_PRODUCT_NOT_AVAILABLE_THIS_OPTION', 'Not available selected option');
define('_JSHOP_X_COMENTAR', 'Comentários (%s)');
define('_JSHOP_INC_PERCENT_TAX', 'Incl. %s %% TAX');
define('_JSHOP_PLUS_PERCENT_TAX', 'plus %s %% TAX');
define('_JSHOP_PLUS_SHIPPING', 'plus <a href="%s" target="_blank">shipping</a>');
define('_JSHOP_SHIPPING', 'Shipping');
define('_JSHOP_DELIVERY_TIME', 'Delivery time');
define('_JSHOP_IDENTIFICATION_NUMBER', 'Identification No.');
define('_JSHOP_TAX_NUMBER', 'Tax No.');
define('_JSHOP_READ_MORE', 'Read more');
define('_JSHOP_MANUFACTURER_INFO', 'Info do fabricante');
define('_JSHOP_MY_ORDERS', 'Os meus pedidos');
define('_JSHOP_FROM', 'de');
define('_JSHOP_TO', 'para');
define('_JSHOP_AND', 'e');
define('_JSHOP_CONFIRM', 'confirmar');
define('_JSHOP_ERROR_MAX_COUNT_ORDER_ONE_PRODUCT', 'Número máximo de artigos para poder realizar o pedido - %s');
define('_JSHOP_ERROR_MIN_COUNT_ORDER_ONE_PRODUCT', 'Número mínimo de artigos para poder realizar o pedido - %s');
define('_JSHOP_ERROR_MIN_SUM_ORDER', 'Preço mínimo do pedido - %s');
define('_JSHOP_ERROR_MAX_SUM_ORDER', 'Preço máximo do pedido - %s');

define('_JSHOP_SELECT','Por favor selecione');
define('_JSHOP_HITS','Vizualizações');
define('_JSHOP_BASIC_PRICE', 'Preço básico');

define('_JSHOP_PAGE_NOT_FOUND', 'Página não encontrada');
define('_JSHOP_HINAME', 'Oi %s');
define('_JSHOP_DOWNLOAD_INVOICE', 'Download invoice');

define('_JSHOP_FREE_DISCOUNT', 'Balance on the coupon'); 
define('_JSHOP_FROM_PRICE_SHIPPING_FREE', 'from %s Free Shipping');
define('JSHP_ST_', 'pcs.');

define('_JSHOP_CLEAR_FILTERS', 'Reset Filter');
define('_JSHOP_VENDOR', 'Vendor');
define('_JSHOP_VIEW_OTHER_VENDOR_PRODUCTS', 'View other vendor products');
define('_JSHOP_ABOUT_VENDOR', 'About vendor');

define('_JSHOP_LAST_PRODUCTS','Last products');
define('_JSHOP_RANDOM_PRODUCTS','Random products');
define('_JSHOP_BESTSELLER','Bestseller');
define('_JSHOP_LABEL','Label');
define('_JSHOP_TOP_RATING','Top rating');
define('_JSHOP_TOP_HITS','Top hits');
define('_JSHOP_PRINT','Print');

define('_JSHOP_DEFAULT_PRICE','Price for Basic members');
define('_JSHOP_QTY_IN_STOCK','Quantity in stock');
define('_JSHOP_UNLIMITED','Unlimited');
define('_JSHOP_OF','of');

define('_JSHOP_SEARCH_FOR','Search for');
define('_JSHOP_ANY_WORDS','Any words');
define('_JSHOP_ALL_WORDS','All words');
define('_JSHOP_EXACT_WORDS','Exact Phrase');

define('_JSHOP_ORDER_DETAILS','Detalhes do pedido');
define('_JSHOP_CART_EMPTY','Seu carrinho está vazio.');
define('_JSHOP_ORDER_DELIVERY_TIME', 'Prazo de entrega');
define('_JSHOP_GROUP', 'Grupo');
define('_JSHOP_TITLE', 'Título');
define('_JSHOP_DISCOUNT', 'Discount');
define('_JSHOP_USER_GROUPS_INFO', 'Todos grupos');
define('_JSHOP_BILL_ADDRESS', 'Endereço de cobrança');
define('_JSHOP_ZOOM_IMAGE', 'Zoom image');
define('_JSHOP_INVOICE_DATE','Invoice Date');
define('_JSHOP_PRIVACY_STATEMENT','Privacy Statement');
define('_JSHOP_PACKAGE_PRICE','Package price');
define('_JSHOP_DELIVERY_DATE','Delivery date');
define('_JSHOP_ERROR_AGB','Read and check "Terms of Service"');
define('_JSHOP_ERROR_SHIPPING','Error shipping');
define('_JSHOP_M_NAME', 'Middle name');
define('_JSHOP_BIRTHDAY', 'Birthday');
define('_JSHOP_REGWARN_MNAME','Please enter your middle name.');
define('_JSHOP_REGWARN_BIRTHDAY','Please enter your birthday.');
define('_JSHOP_REGWARN_MNAME_DELIVERY','Please enter your delivery middle name.');
define('_JSHOP_REGWARN_BIRTHDAY_DELIVERY','Please enter your delivery birthday.');

define('_JSHOP_USER_NUMBER','Número do cliente');
define('_JSHOP_NO_RETURN_DESCRIPTION','I agree explicitly that you start before the end of the withdrawal period with the execution of the contract. I am aware that I have with the start of execution of the contract loses by this consent my withdrawal.');
define('_JSHOP_NO_PRODUCTS_AFTER_FILTER','No related product');

?>
